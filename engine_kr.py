...
# 최대 점수 상수 (A:40 + B:30 + C:30 + D:40 + E:15 + F:15 = 170)
MAX_SCORE = 170

# 시총 필터 기준 (사전 필터 ↔ 1단계 필터 일치)            # [FIX] 1000/2000 불일치 해소
MKTCAP_MIN  = 2000   # 억원
MKTCAP_MAX  = 30000  # 억원
...
def get_investor_detail(ticker, start, end):
    """pykrx 수급 조회 — KRX 차단 시 fdr fallback"""           # [FIX] fallback 추가
    df = None
    try:
...
def score_stock(df, inv_df, cols, inst_streak, for_streak):
    ...
    green_days = int((obv_diff > 0).sum())                     # [FIX] < 0 → > 0 (OBV 상승일 = 매집)
    b = 0
...
def run_kr_scan():
    ...
    # 시총 사전 필터 (MKTCAP_MIN ~ MKTCAP_MAX)                  # [FIX] 기준 상수화
        ...
        for market in ["KOSPI", "KOSDAQ"]:
            ...
            if cap_col:
                for _, row in df_cap.iterrows():
                    ...
                    if cap >= MKTCAP_MIN:                       # [FIX] 1000→MKTCAP_MIN(2000)
    ...
    def scan_ticker_stage1(ticker):
        time.sleep(0.05)                                        # [FIX] 50ms throttle — rate limit 방지
        try:
            ...
            # ② 시총 필터 (MKTCAP_MIN ~ MKTCAP_MAX)            # [FIX] 상수 사용
            ...
            if mktcap is None or not (MKTCAP_MIN <= mktcap <= MKTCAP_MAX): return
            ...
            with lock:
                ...
                passed_stage1.append((ticker, df, vol_ma20, mktcap, rebound_pct, cur,
                                      vol.iloc[-1], vol_ma20.iloc[-1], silence_ratio))  # [FIX] silence_ratio 전달
        except Exception:
            return

    print(f"🔍 1단계: 병렬 가격 스크리닝 (5스레드)...")        # [FIX] 10→5 worker
    ...
    for row in passed_stage1:
        ticker, df, vol_ma20, mktcap, rebound_pct, cur, cur_vol, cur_vm20, silence_ratio = row  # [FIX] unpack
        try:
    ...
    for rank, (total_score, ticker, breakdown, meta) in enumerate(top_raw, 1):
        ...
        final_picks.append({
            ...
            "meta": {
                ...
                "silence_ratio": meta.get('silence_ratio', 0),  # [FIX] 실값 반영
